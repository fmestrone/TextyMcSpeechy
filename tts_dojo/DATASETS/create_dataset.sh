...
#    2. copy your audio files and metadata.csv file into the my_dataset folder. (keep backups elsewhere - this script will change the files)
...
process_audio_file() {
  ...
  if [[ "$(basename "$file")" =~ ^_{0,3}(roomtone|(background)?_{0,3}noise)\.wav ]]; then
    echo "Skipping background noise file: $file"
    return 0
  fi
